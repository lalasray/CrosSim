...
# Loss Computation
def compute_total_loss(pose_embeddings,imu_embeddings_grav):
...
def train_bipose(epochs=300, batch_size=256, learning_rate=0.001, early_stop_patience=80, patience=20, patience_factor=0.5, h5_file_path = "../CrosSim_Data/UniMocap/full_dataset.h5"):
    ...
    for epoch in range(epochs):
        ...
        for text_data, pose_data, imu_data, imu_data_grav in tqdm(dataloader, desc=f"Epoch {epoch+1}/{epochs}"):
            ...
            total_loss = compute_total_loss(pose_embeddings, imu_emb_grav)
            total_loss.backward()
    ...
    torch.save(model.state_dict(), "imupose_11.pth")
